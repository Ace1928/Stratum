...
def run_stellar_collapse(
    ...
    for i in range(cfg.grid_w):
        for j in range(cfg.grid_h):
            # Uniform mass density of 1.0
            fabric.rho[i, j] = 1.0
            # Zero initial momentum
            fabric.px[i, j] = 0.0
            fabric.py[i, j] = 0.0
            # Initialise with significant thermal energy to enable interesting dynamics.
            # A high starting temperature ensures some cells exceed fusion/degeneracy thresholds.
            fabric.E_heat[i, j] = 5.0
            fabric.E_rad[i, j] = 0.0
            # Ensure mixture exists and set to 100% stellar species
